...
class AppsService {
  ...
  instanciate(imageId, opts) {
    ...
    return this.getImageInfo(imageId)
      .then(imageInfo => {
        ...
        let volumes = (imageInfo.volumes || []).concat(YaspConst.YASP_DEDICATED_VOLUME);
  ...
  _createEnvFromVars(varsManifest, varsConfig) {
    ...
    // Expose YASP UID/GID for services sockets
    envs.push('YASP_GID='+process.getgid());
    envs.push('YASP_UID='+process.getuid());
  ...
  _findAppContainer(instanceId) {
    ...
    return containersService.listContainers(opts)
      .then(results => {
        if(!results.length) {
          throw new Error(`Canno't found app ${instanceId} !`);
        }
        return results[0];
      })
    ;
