...
class Aegir {
  ...
  checkTemplates() {
    ...
    return appsService.fetchAllTemplates()
      .then(templates => {
        let errorFound = false;
        templates.forEach(function(tpl) {
          try {
            schemas.validate(tpl, schemas.AppTemplate);
          } catch(err) {
            console.error('Invalid template %j. Error: %s', tpl, err.stack);
            errorFound = true;
          }
        });
        if(errorFound) {
          throw new Error('You have invalid app templates.');
        }
      })
    ;
  }
