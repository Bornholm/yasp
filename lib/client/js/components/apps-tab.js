...
class AppsTab extends React.Component {
  ...
  render() {
    ...
    let cards = apps.map(app => {
      ...
      if(!app.isRunning) {
        deleteButton = (
          ...
          <Button bsStyle="danger" bsSize="small" className="pull-right"
            onClick={ this.deleteApp.bind(this, app.instanceId) }>
            Delete
          </Button>
          /* jshint ignore:stop */
        );
      ...
      let footer = (
        ...
        <div>
          <ButtonGroup>
            <Button bsStyle={ app.isRunning ? "warning" : "success" } bsSize="small"
              onClick={ toggleApp }>
  ...
  deleteApp(instanceId) {
    this.props.dispatch(Actions.Apps.deleteApp(instanceId))
      .then(() => {
        this.props.dispatch(Actions.Apps.fetchAppsList());
      })
    ;
