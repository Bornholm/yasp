...
class NetworkGraph extends BaseGraph {
  ...
  constructor() {
    super();
    this.state = {};
    this._formatIORateTicks = this._formatIORateTicks.bind(this);
  ...
  render() {
    ...
    let currentIORate = {
      input: this._getPreferredBytesUnit(ioRate.input[ioRate.input.length-1].y),
      output: this._getPreferredBytesUnit(ioRate.output[ioRate.output.length-1].y)
    };
      ...
      <div className="panel panel-default" style={this.cardStyle}>
        ...
        <div className="panel-body">
          <div style={{background: '#333'}}>
            <VictoryChart
              ...
              padding={{left: 50, right: 20, bottom: 30, top: 20}}>
              <VictoryAxis
                style={this.axisStyle}
                tickFormat={this._formatTimeTicks} />
              <VictoryAxis
                style={this.axisStyle}
                tickFormat={this._formatIORateTicks}
                dependentAxis />
              <VictoryLine
        ...
        <div className="panel-footer clearfix">
          <div className="pull-left">
            <b>{t('input_rate')}:&nbsp;</b><span>{currentIORate.input.value.toFixed(2)}{currentIORate.input.unit}/s</span>
          </div>
          <div className="pull-right">
            <b>{t('output_rate')}:&nbsp;</b><span>{currentIORate.output.value.toFixed(2)}{currentIORate.output.unit}/s</span>
          </div>
        </div>
      </div>
    );
  ...
  _formatIORateTicks(tickValue) {
    let result = this._getPreferredBytesUnit(tickValue);
    let value = result.value.toFixed(result.value >= 100 ? 0 : 1);
