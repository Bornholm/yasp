      ...
      return (
        ...
        <div className="panel panel-default" key={template.id} style={cardStyle}>
          ...
          <div className="panel-footer">
            <Link className="btn btn-primary btn-sm" to={`/instanciate/${template.id}`}>{t('instanciate')}</Link>
          </div>
        </div>
        /* jshint ignore:end */
      );
      ...
      <div>
        <div className="clearfix">
          <div className="btn-group pull-right" role="group">
            <button className="btn btn-info" onClick={this.onRefreshClick}>{t('refresh')}</button>
          </div>
        </div>
        <FlexLayout>
          {cards}
